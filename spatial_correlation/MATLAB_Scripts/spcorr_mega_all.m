...
date = '20250711';
loc_networks = {'language', 'language', 'language', 'language', 'language', 'language', 'MD', 'DMN'};
loc_tasks = {'langloc_DiffTasks_1', 'langloc_DiffTasks_2', 'langloc_DiffTasks_3', 'langloc_DiffTasks_4', 'langloc_DiffTasks_5', 'langloc_DiffTasks_6', 'spatialFIN', 'spatialFIN'};
network = 'language';
tasks = {'langloc_DiffTasks_1', 'langloc_DiffTasks_2', 'langloc_DiffTasks_3',...
...
for c = 1:8    % Localizer index
    loc_network = loc_networks{c};
    loc_task = loc_tasks{c};

    if strcmp(loc_network, 'language')
        orth1_loc_file = 'locT_0028_percentile-ROI-levd6892c28219934d15f6618885cfc9f46_f2f281a0bb9ffa54f781c09b52273cdf.ROIs.nii';
        orth2_loc_file = 'locT_0030_percentile-ROI-leva8e90a457ebfca647247c87aa7a52ef8_f2f281a0bb9ffa54f781c09b52273cdf.ROIs.nii';
        loc_file = 'locT_0003_percentile-ROI-lev1534d2f6481735600ae7d49107198726_f2f281a0bb9ffa54f781c09b52273cdf.ROIs.nii';
    elseif strcmp(loc_network, 'MD')
        loc_file = 'locT_0003_percentile-ROI-lev85603954b0894e68c746d625a847e070_cd77924fb88b48a24e07f273d184ac3b.ROIs.nii';
    elseif strcmp(loc_network, 'DMN')
        loc_file = 'locT_0004_percentile-ROI-lev268c303b50c8d0776e48df0425d14a0d_3098a112bdcbcdfd6713c2647fac6d72.ROIs.nii';
    else
    ...
    for i=1:length(tasks)
        ...
        for j=i:length(tasks)
            ...
            if c >= 7    % MD or DMN localizer in use
                if i == j
                    loc_files = {loc_file};
                    loc_labels = {'All'};
                    contrasts1_list = {{'ODD_S', 'ODD_N'}};
                    contrasts2_list = {{'EVEN_S', 'EVEN_N'}};
                else
                    loc_files = {loc_file};
                    loc_labels = {'All'};
                    contrasts1_list = {{'ODD_S', 'ODD_N', 'EVEN_S', 'EVEN_N'}};
                    contrasts2_list = {{'EVEN_S', 'EVEN_N', 'ODD_S', 'ODD_N'}};
                end
                    
            elseif i == j && c == i % Language localizer = effect 1 task = effect 2 task; cannot evaluate
                continue

            elseif i == j
                loc_files = {orth1_loc_file, orth2_loc_file};
                loc_labels = {'2', '1'};
                contrasts1_list = {{'ODD_S', 'ODD_N'}, {'ODD_S', 'ODD_N'}};
                contrasts2_list = {{'EVEN_S', 'EVEN_N'}, {'EVEN_S', 'EVEN_N'}};

            elseif c == i
                loc_files = {orth1_loc_file, orth2_loc_file};
                loc_labels = {'2', '1'};
                contrasts1_list = {{'ODD_S', 'ODD_N'}, {'EVEN_S', 'EVEN_N'}};
                contrasts2_list = {{'ODD_S', 'ODD_N', 'EVEN_S', 'EVEN_N'}, {'ODD_S', 'ODD_N', 'EVEN_S', 'EVEN_N'}};

            elseif c == j
                loc_files = {orth1_loc_file, orth2_loc_file};
                loc_labels = {'2', '1'};
                contrasts2_list = {{'ODD_S', 'ODD_N'}, {'EVEN_S', 'EVEN_N'}};
                contrasts1_list = {{'ODD_S', 'ODD_N', 'EVEN_S', 'EVEN_N'}, {'ODD_S', 'ODD_N', 'EVEN_S', 'EVEN_N'}};
            ...
            for k = 1:length(loc_files)
                loc_file_i = loc_files{k};
                loc_label_i = loc_labels{k};
                contrasts1 = contrasts1_list{k};
                contrasts2 = contrasts2_list{k};

                output_pth = ['/nese/mit/group/evlab/u/ruiminga/DiffTasks/spcorr_DiffTasks/spcorr_' loc_network '_' loc_task '_'...
                    task1 '_' task2 '_' loc_label_i '_' date]; 
                ...
                subjects_loc = rowfun(@(uid, session) make_roi_paths(data_dir, loc_task, uid, session, loc_file_i),...
                    subject_info_loc, "OutputVariableNames", "ROIpath");
                rois = cellstr(subjects_loc.ROIpath');

                subject_info_task1 = [rowfun(@(x) sprintf("%03d", x), session_info_thisanalysis(:,"UID")),...
                    session_info_thisanalysis(:,task1)];
                subjects_main = rowfun(@(uid, session) make_spm_paths(data_dir, task1, uid, session),... 
                    subject_info_task1, "OutputVariableNames", "SPMpath");
                spmfiles_task1 = cellstr(subjects_main.SPMpath');

                subject_info_task2 = [rowfun(@(x) sprintf("%03d", x), session_info_thisanalysis(:,"UID")),...
                    session_info_thisanalysis(:,task2)];
                subjects_main = rowfun(@(uid, session) make_spm_paths(data_dir, task2, uid, session),...
                    subject_info_task2, "OutputVariableNames", "SPMpath");
                spmfiles_task2 = cellstr(subjects_main.SPMpath');


                bcc=struct(...
                    'swd',output_pth,...   % output directory
                    'EffectOfInterest1_spm', {spmfiles_task1},...         
                    'EffectOfInterest2_spm', {spmfiles_task2},...
                    'EffectOfInterest1_contrasts',{contrasts1},...
                    'EffectOfInterest2_contrasts',{contrasts2},... 
                    'ManualROIs', {rois},...
                    'overwrite', 1, ...
                    'model',1,...   
                    'ExplicitMasking', [], ...
...
function [spm_path] = make_roi_paths(data_dir, expt, uid, session, roi_file)
    session = strcat(uid, '_', session{:}, '_PL2017');
    spm_path = fullfile(data_dir, session, ['firstlevel_' expt], roi_file);
end
        ...
        for np=1:size(bcc.EffectOfInterest1_spm,2),%subjects
            ...
            for nexp=1:numel(spm_data.SPMfiles),
                ...
                if ~isempty(iIc1)&&~isempty(iIc2),        
                    ...
                    % OVERRIDE HERE: replace all oo with 1
                    oo = ones(size(oo));
